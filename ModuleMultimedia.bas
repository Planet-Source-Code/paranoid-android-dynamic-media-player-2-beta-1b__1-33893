...
Dim glo_from As Long ':( Missing Scope
Dim glo_to As Long ':( Missing Scope
Dim glo_AliasName As String ':( Missing Scope
Dim glo_hWnd As Long ':( Missing Scope
...
Public Function AreMultimediaAtEnd(AliasName As String, lastFrame As Long) As Boolean
  ...
  Dim currpos As Long
    ...
    If lastFrame = 0 Then lastFrame = GetTotalframes(AliasName) ':( Expand Structure
...
Public Function GetPercent(AliasName As String) As Long
  ...
  'if it successed and if the function failed will return value -1
    ...
    Dim TotalFrames As Long ':( Move line to top of current Function
    Dim currframe As Long ':( Move line to top of current Function
      TotalFrames = GetTotalframes(AliasName)
...
Public Function GetSize(AliasName As String, CxOrCy As String) As Long
  ...
  'or if not will return value long is -1
    ...
    If Not LCase$(CxOrCy) = "cx" And Not LCase$(CxOrCy) = "cy" Then GetSize = -1: Exit Function ':( Expand Structure or consider reversing Condition
  Dim dwReturn As Long ':( Move line to top of current Function
  Dim size As String * 128 ':( Move line to top of current Function
  Dim s1, s2, s3, Width, Height As Long ':( As Variant ?':( Move line to top of current Function
    ...
    s2 = InStr(s1 + 1, size, " ")
    s1 = InStr(s2 + 1, size, " ")
    Width = Mid$(size, s2, s1 - s2)
    Height = Mid$(size, s1 + 1)

    'Success
    If LCase$(CxOrCy) = "cx" Then 'get the width
        GetSize = Width
      ElseIf LCase$(CxOrCy) = "cy" Then 'Get the height'NOT LCASE$(CXORCY)...
        GetSize = Height
    End If
...
Public Function GetStatusMultimedia(AliasName As String) As String
  ...
  Dim i As Integer ':( Move line to top of current Function
  Dim CharA As String ':( Move line to top of current Function
  Dim RChar As String ':( Move line to top of current Function
    RChar = Right$(Status, 1)
    For i = 1 To Len(Status)
        CharA = Mid$(Status, i, 1)
        If CharA = RChar Then Exit For ':( Expand Structure or consider reversing Condition
...
Public Function PlayMultimedia(AliasName As String, from_where As String, to_where As String) As String
  ...
  'or if not will return value string description the error which occur
    ...
    If from_where = vbNullString Then from_where = 0 ':( Expand Structure
    If to_where = vbNullString Then to_where = GetTotalframes(AliasName) ':( Expand Structure
  ...
  Dim cmdToDo As String * 128 ':( Move line to top of current Function
  Dim dwReturn As Long ':( Move line to top of current Function
  Dim ret As String * 128 ':( Move line to top of current Function
...
Public Function PutMultimedia(hwnd As Long, AliasName As String, left As Long, top As Long, Width As Long, Height As Long) As String
  ...
  Dim ret As String * 128
    ...
    If Width = 0 Or Height = 0 Then
        'Get Window Size
  Dim rec As RECT ':( Move line to top of current Function
...
Public Function SetAutoRepeat(hwnd As Long, AliasName As String, first_frame As String, last_frame As String, autoTrueOrFalse As Boolean) As Boolean
  ...
  Dim Result As String
    ...
    If first_frame = vbNullString Then first_frame = 0 ':( Expand Structure
    If last_frame = vbNullString Then last_frame = GetTotalframes(AliasName) ':( Expand Structure
...
Sub TimerFunction()
  ...
  Dim Result As String
    ...
    If currpos = -1 Then Exit Sub   'if  function get cuurent pos not success then exit':( Expand Structure or consider reversing Condition
    '
    If Val(glo_to) = currpos Or (Val(glo_to) - 1) < currpos Then
        Result = PlayMultimedia(glo_AliasName, Str(glo_from), Str(glo_to))
        If Not Result = "Success" Then KillTimer glo_hWnd, 500 'if  function play not success then kill timer':( Expand Structure
    End If
